...
class SystemConfig(models.Model):
    ...
    days_new_item = models.PositiveIntegerField(
        ...
        help_text="Note: This will be used to add the 'new' tag to the product. Example if this as a value of 7 means "
                  "any "
    ...
    days_best_seller = models.PositiveIntegerField(
        ...
        help_text="Note: This will be used to auto select products based on units sold of products in last X days. For "
    ...
    add_tip_post_delivery_days = models.PositiveIntegerField(
        ...
        help_text="Note: This config will be used to control how many days after delivery the customer can increase "
                  "tip. "
    ...
    points_dollar_conversion = models.PositiveIntegerField(
        ...
        help_text="Note: X points earned on each dollar for order placed. Ex: X = 1, all customers including super "
                  "members will earn 1 points for each dollar for order placed. "
    )
    referer_sharing_cutoff_days = models.PositiveIntegerField(
        null=True, blank=True, verbose_name="Referer Sharing Cutoff Days.",
        help_text="Note: After order is delivered or picked up."
    )
    ...
    max_wholesale_threshold_to_sell = models.PositiveIntegerField(
        ...
        help_text="Note: This value will be in percentage. This is the setting to calculate the max inventory from "
    ...
    max_orders_for_review = models.PositiveIntegerField(
        ...
        help_text="Note: This will be used on the 'my reviews' screen in my account to show hte no. of orders for "
                  "review. "
    )
    mobile_app_landing_screen_seq = models.CharField(
        max_length=30,
        null=True, blank=True, verbose_name="Mobile App Landing Screen Sequence",
        help_text="Note: Screen names: login, signup This will be used to decide which screen will the user see when "
    ...
    checkout_care_disclaimers = models.TextField(
        ...
        help_text="Note: Provide the instruction to user on checkout screen."
    )
    filter_sold_out_from_our_picks = models.BooleanField(
        default=False,
        verbose_name="Is Filter Sold Out From Our Picks?",
        help_text="Note: 'Check' means filter out all out of stock items,  'Uncheck' means show all out of stock of "
                  "item in put picks. "
    )
